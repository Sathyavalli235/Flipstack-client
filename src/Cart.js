...
const Cart = () => {
  ...
  const updateQuantity = async (cartId, quantity) => {
    try {
      await axios.put(
        `https://flipstack-server.onrender.com/api/cart/update/${cartId}`,
        { quantity }
      );
      setCartItems((prev) =>
        prev.map((item) =>
  ...
  const removeFromCart = async (id) => {
    try {
      await axios.delete(
        `https://flipstack-server.onrender.com/api/cart/delete/${id}`
      );
  ...
  <div className="cart-container">
    ...
    {cartItems.length === 0 ? (
      ...
      <>
            ...
            <div className="cart-details">
              ...
              <div className="cart-actions">
                <div className="qty-controls">
                  ...
                  <span className="qty-display">{item.product.quantity}</span>
        ...
        {/* Cart Buy Now button */}
        <button className="cart-buy-btn" onClick={() => navigate("/payment")}>
          Buy Now
        </button>
      </>
    )}
      ...
      <footer className="footer-container">
        <div className="footer-top">
          ...
          <div>
            <h6>Mail Us:</h6>
            <p>
              FlipStack Internet Private Limited,
              <br />
